...
# Publish LogFlux Java SDK to a local public repository directory.
# Extracts sdks/java/ from the monorepo, squashes into a single clean commit,
# and syncs to the local public repo.
#
# The public repo lives at ../logflux-public/logflux-java-sdk/.
# From there you can push to GitHub manually.
#
# Usage:
...
PUBLIC_DIR="$(dirname "$MONO_ROOT")/logflux-public/logflux-java-sdk"
WORK_DIR="$(mktemp -d)"
...
COMMIT_MSG="LogFlux Java SDK"
if [ -n "$TAG" ]; then
    COMMIT_MSG="LogFlux Java SDK $TAG"
fi
NEW_COMMIT="$(echo "$COMMIT_MSG" | git commit-tree "$TREE")"
git reset --hard "$NEW_COMMIT"

# Verify no leaks in the single commit message
...
# Sync to local public directory
if [ -d "$PUBLIC_DIR/.git" ]; then
    ...
    git remote add origin git@github.com:logflux-io/logflux-java-sdk.git
fi
