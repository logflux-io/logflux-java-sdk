 ...
 * 3. POST /v1/handshake/complete -> server returns key_id
 */
public final class Handshake {
    ...
    public static Result perform(String handshakeBaseUrl, String apiKey,
        ...
        // Prefer key_id (current API spec), fall back to key_uuid (legacy)
        String keyUuid = Json.getString(completeParsed, "key_id");
        if (keyUuid == null || keyUuid.trim().isEmpty()) {
            keyUuid = Json.getString(completeParsed, "key_uuid");
        }
        if (keyUuid == null || keyUuid.trim().isEmpty()) {
            Map<String, Object> data = Json.getObject(completeParsed, "data");
            if (data != null) {
                keyUuid = Json.getString(data, "key_id");
                if (keyUuid == null || keyUuid.trim().isEmpty()) {
        ...
        if (keyUuid == null || keyUuid.trim().isEmpty()) {
            Arrays.fill(aesKey, (byte) 0);
            throw new RuntimeException("Handshake complete: no key_id in response");
        }
